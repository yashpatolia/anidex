...
// Client-fetched anime detail page body (see anilist-client.ts's file
// comment for the overall architecture). generateMetadata() in
// src/app/anime/[id]/page.tsx still fetches server-side, live and
// uncached, via src/lib/anilist.ts — crawlers can't run this component at
// all, so that's the one piece that stays server-side; everything a real
// visitor sees comes from here instead.
...
// Mirrors AddToListControl's own local WatchStatus union — that component
// deliberately doesn't import the generated Prisma enum (a server-only
// module that would break the client bundle), so this matches its literal
// string union instead of using `string`, which would've silently
// widened past whatever AddToListControl actually accepts.
...
export function AnimeDetailView({ anilistId }: { anilistId: number }) {
  const { data: session, status: sessionStatus } = useSession();
  const [anime, setAnime] = useState<AnilistMediaDetail | null | undefined>(undefined);
